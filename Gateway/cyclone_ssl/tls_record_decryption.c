   ...
   TlsEncryptionEngine *decryptionEngine, void *record)
{
   ...
   if(length < (decryptionEngine->recordIvLen + decryptionEngine->authTagLen)){
       TRACE_ERROR("MESSAGE LENGTH IS NOT GOOD TLS DECRYPT\r\n  We have a length of the IV of: %d and a auth of: %d",decryptionEngine->recordIvLen,decryptionEngine->authTagLen);
   ...
   if(error){
       TRACE_ERROR("WRONG TAG TLS RECORD\r\n");
      return ERROR_BAD_RECORD_MAC;
   }
   ...
   TlsEncryptionEngine *decryptionEngine, void *record)
{
   ...
   //Return status code
   if(bad){
       TRACE_ERROR("BAD CBC TLS RECORD\r\n");
      return ERROR_BAD_RECORD_MAC;
   }
   else
      return NO_ERROR;
   ...
   TlsEncryptionEngine *decryptionEngine, void *record)
{
   ...
   if(length < hashAlgo->digestSize){
       TRACE_ERROR("BAD LENGTH TLS RECORD\r\n");
      return ERROR_BAD_RECORD_MAC;
   }
   ...
   if(mask != 0){
      TRACE_ERROR("iNVALID CODE TLS DECRIPTION\r\n");
      return ERROR_BAD_RECORD_MAC;
   }
   else
      return NO_ERROR;
