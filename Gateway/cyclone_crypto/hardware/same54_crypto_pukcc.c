...
error_t rsadp(const RsaPrivateKey *key, const Mpi *c, Mpi *m)
{
      ...
      key->dp.size && key->dq.size && key->qinv.size)
   {
      ...
      n = mpiGetByteLength(c);

      //Retrieve the length of the primes, in bytes
   ...
   const uint8_t *digest, size_t digestLen, EcdsaSignature *signature)
{
   ...
   n = mpiGetBitLength(&ecParams->q);

   //Generated a pseudorandom number
   error = mpiRand(&k, n, prngAlgo, prngContext);

   //Check error code
   if(!error)
   {
      //Make sure that 0 < k < q
      if(mpiComp(&k, &ecParams->q) >= 0)
         mpiShiftRight(&k, 1);
   ...
   if(!error)
   {
      ...
      params.a = pukccImportMpi(&pos, &ecParams->a, modLen + 4);
      //Copy private key
      params.privateKey = pukccImportMpi(&pos, privateKey, scalarLen + 4);
      //Copy random scalar
   ...
   const EcdsaSignature *signature)
{
   ...
   if(PUKCL(u2Status) == PUKCL_OK)
   {
      ...
      //Copy public key
      params.publicKey.x = pukccImportMpi(&pos, &publicKey->x, modLen + 4);
      params.publicKey.y = pukccImportMpi(&pos, &publicKey->y, modLen + 4);
